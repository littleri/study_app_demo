...
test.describe("local DemoRepository P0 flow", () => {
  ...
  test("replays upload through report with deterministic local data only", async ({ page }) => {
    ...
    await clickUniqueAction(page, page.getByRole("button", { name: "加入错题本", exact: true }), "open the mistake book");
    ...
    await clickUniqueAction(page, page.getByRole("button", { name: "查看答案", exact: true }), "reveal the flashcard answer");
    await expect(page.locator(".memory-reveal")).toHaveAttribute("aria-pressed", "true");
    await clickUniqueAction(page, page.getByRole("button", { name: "回到章节", exact: true }), "return to the lesson");
