...
async function openLesson(page: Page) {
  await expandCurrentStudySection(page);
  const flashcards = page.locator('#study-section-c2s1-content .study-tool-card[data-tool="flashcards"]');
  await flashcards.focus();
  await expect(flashcards).toBeFocused();
  await flashcards.press("Enter");
...
test.describe("current DemoRepository responsive matrix", () => {
  test("keeps home, navigation, safe chrome, and primary actions usable in both mapped viewports", async ({ page }, testInfo) => {
    ...
    await expectReachable(page.locator(".home-tool-button").first(), `${project.name} paired upload tool`);
