...
const probeOverlay = process.argv.includes("--probe-overlay");
const probeComposition = process.argv.includes("--composition-probe");
...
async function greenLayout(page) {
  const css = [`:root,html,body,#root{margin:0!important;width:100%!important;height:100%!important;overflow:hidden!important;background:${green}!important}`, `.device-preview-studio,.device-preview-canvas-area,.device-preview-canvas{background:${green}!important;box-shadow:none!important;overflow:visible!important}`, `.device-preview-studio{width:100vw!important;height:100vh!important;display:block!important;padding:0!important}`, `.device-preview-canvas-area{position:fixed!important;inset:0!important;width:1440px!important;height:2880px!important;display:block!important;padding:0!important;margin:0!important;transform:none!important}`, `.device-preview-canvas{position:absolute!important;left:0!important;top:0!important;width:1440px!important;height:2880px!important;margin:0!important;transform:none!important}`, `.device-preview-toolbar,.device-preview-output-summary,.device-preview-status-announcer{display:none!important}`, `.device-preview-frame--iphone-17-pro,.device-preview-frame--iphone-17-pro::before,.device-preview-frame--iphone-17-pro::after{background:transparent!important;border:none!important}`, `.device-preview-frame--iphone-17-pro{position:absolute!important;left:50%!important;top:50%!important;margin:0!important;transform:translate(-50%,-50%) scale(2.70)!important;transform-origin:center center!important;box-shadow:none!important}`, `*,*::before,*::after{cursor:none!important}`].join("\n");
...
async function overlay(frame) {
  await frame.locator("body").evaluate(() => {
    const s = document.createElement("style"); s.id = "promo-purple-ring-style"; s.textContent = `#promo-ring,#promo-ripple{display:block!important;position:fixed!important;z-index:2147483647!important;pointer-events:none!important;border-radius:999px!important;left:0;top:0;opacity:0;transform:translate(-50%,-50%);transition:opacity .16s ease,transform .42s cubic-bezier(.22,1,.36,1)}#promo-ring{width:38px!important;height:38px!important;border:4px solid #8B5CFF!important;background:transparent!important;box-shadow:0 0 0 3px rgba(255,255,255,.92),0 0 22px rgba(111,66,255,.8)!important}#promo-ring.on{opacity:1!important}#promo-ring.press{transform:translate(-50%,-50%) scale(.78)!important}#promo-ripple{width:42px!important;height:42px!important;border:4px solid rgba(139,92,255,.96)!important;background:transparent!important}#promo-ripple.go{animation:promoRipple .58s cubic-bezier(.16,1,.3,1) both!important}@keyframes promoRipple{0%{opacity:1;transform:translate(-50%,-50%) scale(.72)}100%{opacity:0;transform:translate(-50%,-50%) scale(3.2)}}[data-promo-target]{position:relative!important;isolation:isolate!important;outline:3px solid rgba(139,92,255,.94)!important;outline-offset:3px!important;box-shadow:0 0 0 7px rgba(139,92,255,.26)!important}[data-promo-target]::after,[data-promo-target]::before{content:""!important;display:block!important;position:absolute!important;left:50%!important;top:50%!important;pointer-events:none!important;border-radius:999px!important;z-index:2147483647!important;background:transparent!important}[data-promo-target]::after{width:38px!important;height:38px!important;border:4px solid #8B5CFF!important;box-shadow:0 0 0 3px rgba(255,255,255,.92),0 0 22px rgba(111,66,255,.8)!important;transform:translate(-50%,-50%)!important}[data-promo-target]::before{width:42px!important;height:42px!important;border:4px solid rgba(139,92,255,.96)!important;opacity:0;transform:translate(-50%,-50%) scale(.72)!important}[data-promo-target][data-promo-pressed]::before{animation:promoTargetRipple .58s cubic-bezier(.16,1,.3,1) both!important}@keyframes promoTargetRipple{0%{opacity:1;transform:translate(-50%,-50%) scale(.72)}100%{opacity:0;transform:translate(-50%,-50%) scale(3.2)}}`; document.head.append(s);
    for (const id of ["promo-ring", "promo-ripple"]) { const e = document.createElement("div"); e.id = id; e.setAttribute("aria-hidden", "true"); document.body.append(e); }
  });
}
async function cue(locator, label) {
  ...
  await frame.locator("body").evaluate((_body, { x, y }) => { for (const id of ["promo-ring", "promo-ripple"]) { const e = document.querySelector("#" + id); e.style.left = x + "px"; e.style.top = y + "px"; } document.querySelector("#promo-ring").className = "on"; }, point);
  const evidenceIndex = clicks.length + 1;
  const evidenceBase = `${activeSection || "unknown"}-interaction-${String(evidenceIndex).padStart(2, "0")}-${Date.now()}`;
  const ringEvidence = path.join(movies, `${evidenceBase}-ring.png`);
  const rippleEvidence = path.join(movies, `${evidenceBase}-ripple.png`);
  if (probeOverlay) { await page.screenshot({ path: path.join(movies, "03-08-overlay-probe-ring.png"), type: "png" }); const ringState = await frame.locator("#promo-ring").evaluate((e) => { const c = getComputedStyle(e); const r = e.getBoundingClientRect(); return { className:e.className, display:c.display, opacity:c.opacity, visibility:c.visibility, zIndex:c.zIndex, left:c.left, top:c.top, width:r.width, height:r.height, x:r.x, y:r.y }; }); console.log("[overlay-probe] " + JSON.stringify(ringState)); }
  if (!rehearsal) await page.screenshot({ path: ringEvidence, type: "png" });
  await sleep(500); await frame.locator("body").evaluate(() => { document.querySelector("#promo-ring").className = "on press"; const r = document.querySelector("#promo-ripple"); r.className = ""; void r.getBoundingClientRect(); r.className = "go"; document.querySelector("[data-promo-target]")?.setAttribute("data-promo-pressed", ""); });
  if (probeOverlay) { await page.screenshot({ path: path.join(movies, "03-08-overlay-probe-ripple.png"), type: "png" }); throw new Error("overlay probe complete"); }
  await sleep(130); if (!rehearsal) await page.screenshot({ path: rippleEvidence, type: "png" });
  await sleep(160);
  await locator.click(); clicks.push({ label, atSeconds: +((now() - started) / 1000).toFixed(3), ringEvidence: rehearsal ? null : ringEvidence, rippleEvidence: rehearsal ? null : rippleEvidence }); await sleep(350); await frame.locator("body").evaluate(() => { document.querySelector("#promo-ring").className = ""; document.querySelectorAll("[data-promo-target]").forEach((x) => x.removeAttribute("data-promo-target")); });
}
...
function start(id) { activeSection = id; sectionTimes.set(id, { start: now(), end: null }); console.log("[segment-start] " + id); }
...
try {
  ...
  if (probeComposition) {
    const shell = await page.locator('[data-testid="device-preview-frame"]').evaluate((e) => { const toRect=(r)=>r&&({x:r.x,y:r.y,width:r.width,height:r.height,left:r.left,right:r.right,top:r.top,bottom:r.bottom}); const r=e.getBoundingClientRect(); const b=e.querySelector(".device-preview-bezel")?.getBoundingClientRect(); const controls=Array.from(e.querySelectorAll("[data-hardware-control]")).map((node)=>({id:node.getAttribute("data-hardware-control"),...toRect(node.getBoundingClientRect())})); const unionRects=[b,...Array.from(e.querySelectorAll("[data-hardware-control]")).map((node)=>node.getBoundingClientRect())].filter(Boolean); const left=Math.min(...unionRects.map((v)=>v.left)); const right=Math.max(...unionRects.map((v)=>v.right)); const top=Math.min(...unionRects.map((v)=>v.top)); const bottom=Math.max(...unionRects.map((v)=>v.bottom)); const canvas=e.closest(".device-preview-canvas")?.getBoundingClientRect(); const area=e.closest(".device-preview-canvas-area")?.getBoundingClientRect(); return { ...toRect(r), bezel:toRect(b), hardwareControls:controls, hardwareUnion:{left,right,top,bottom,x:left,y:top,width:right-left,height:bottom-top}, canvas:toRect(canvas), area:toRect(area), viewport:{width:innerWidth,height:innerHeight} }; });
    const bounds = shell.hardwareUnion || shell.bezel || shell;
    const margins = { left: bounds.left, right: shell.viewport.width - bounds.right, top: bounds.top, bottom: shell.viewport.height - bounds.bottom };
    await page.screenshot({ path: path.join(movies, "03-08-composition-probe.png"), type: "png" });
    await writeFile(path.join(movies, "03-08-composition-probe.json"), JSON.stringify({ shell, margins, centered: { horizontalError: Math.abs(margins.left - margins.right), verticalError: Math.abs(margins.top - margins.bottom) } }, null, 2) + "\n", "utf8");
    console.log("[composition-probe] " + JSON.stringify({ shell, margins }));
    throw new Error("composition probe complete");
  }
  ...
  await action("05-processing", async () => {}, async () => visible(frame.locator(".chapter-confirm-screen"), 90000), async () => { const p = frame.locator(".progress-wrap"); for (const n of ["18%", "46%", "74%", "100%"]) { await p.getAttribute("aria-label").catch(() => ""); await sleep(1300); } /* This deterministic build auto-enters the directory at 100%; do not invent a non-existent click. */ await visible(frame.locator(".chapter-confirm-screen"), 90000); });
